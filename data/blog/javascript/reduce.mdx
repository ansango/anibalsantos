...
el `acumulador` es el valor anterior, resultante de la operacion que se devuelve, el `numero` es el elemento actual de la iteración, todo esto es pasado como primer argumento de la función. Como segundo argumento tenemos el valor inicial sobre el cual se comienza.
